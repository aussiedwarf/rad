...
impl RendererDirectX12 {
    ...
    pub fn new(
        ...
        let result_swap_chain =
            unsafe { factory.CreateSwapChain(&queue, &swap_chain_desc, &mut swap_chain) };
        ...
        let description_heap = match unsafe {
            device.CreateDescriptorHeap::<ID3D12DescriptorHeap>(&render_target_view_heap_desc)
        } {
            Ok(res) => res,
            Err(_res) => return Err(RendererError::Error),
        };
        ...
        let mut render_target_view_handle: D3D12_CPU_DESCRIPTOR_HANDLE =
            unsafe { description_heap.GetCPUDescriptorHandleForHeapStart() };

        // Get the size of the memory location for the render target view descriptors.
        let render_target_view_descriptor_size =
        ...
        let command_allocator = match unsafe {
        ...
        */
        ...
        let command_list: ID3D12GraphicsCommandList = match unsafe {
            device.CreateCommandList //::<ID3D12CommandAllocator,ID3D12PipelineState,ID3D12GraphicsCommandList>
