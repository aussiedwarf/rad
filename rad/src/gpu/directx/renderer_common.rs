...
#[cfg(windows)]
fn get_default_adapter_by_gpu_preference(
    a_dxgi_factory: &IDXGIFactory6,
    a_gpu_preference: DXGI_GPU_PREFERENCE,
) -> Result<IDXGIAdapter1, RendererError> {
    for i in 0.. {
        match unsafe {
...
#[cfg(windows)]
pub fn get_adapter(
    a_factory: &IDXGIFactory6,
    a_device_type: DeviceType,
) -> Result<IDXGIAdapter1, RendererError> {
    match a_device_type {
        DeviceType::Default => return get_default_adapter(&a_factory),
        DeviceType::HighPerformance => {
            return get_default_adapter_by_gpu_preference(
                &a_factory,
