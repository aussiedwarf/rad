...
impl Swapchain {
    pub fn new(
        ...
        a_extent: ash::vk::Extent2D,
    ) -> Result<Self, RendererError> {
        let present_modes: Vec<ash::vk::PresentModeKHR> = unsafe {
            match a_surface.surface.get_physical_device_surface_present_modes(
                a_physical_device.device,
                a_surface.surface_khr,
            ) {
                Ok(res) => res,
        ...
        let surface_capabilities = unsafe {
            match a_surface.surface.get_physical_device_surface_capabilities(
                a_physical_device.device,
                a_surface.surface_khr,
            ) {
                Ok(res) => res,
        ...
        let swapchain_base = SwapchainBase {
            swapchain: swapchain,
            swapchain_loader: swapchain_loader,
        };

        let swapchain_images = unsafe {
            match swapchain_base
                .swapchain_loader
        ...
        for &image in swapchain_images.iter() {
            image_views.push(
                match ImageView::new(a_logical_device.clone(), &image, a_format.format) {
                    Ok(res) => res,
                    Err(_res) => return Err(RendererError::Error),
                },
            );
    ...
    fn get_swap_extent(
        a_capabilities: &ash::vk::SurfaceCapabilitiesKHR,
        a_extents: ash::vk::Extent2D,
    ) -> ash::vk::Extent2D {
    ...
    pub fn clear(&mut self) {
        ...
        for image_view in self.image_views.iter_mut() {
            unsafe {
                self.logical_device
                    .device
                    .destroy_image_view(image_view.view, None)
            };
