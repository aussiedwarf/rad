    ...
    fn load_texture(&mut self, a_image: &image::DynamicImage, a_texture: Arc<dyn Texture>) {
        ...
        unsafe {
            ...
            gl::TexImage2D(
                ...
                gl::RGBA,
                gl::UNSIGNED_BYTE,
                rgba.as_bytes().as_ptr() as *const std::os::raw::c_void,
            );
                        ...
                        ContainerType::Mat2x2 => {
                            ...
                            unsafe {
                                gl::UniformMatrix2fv(
                                    ...
                                    1,
                                    0,
                                    &data.to_cols_array()[0] as *const f32,
                                );
                            }
                        }
...
fn get_gl_version_minor(
    a_version_major: i32,
    a_version_minor: VersionNum,
) -> Result<i32, RendererError> {
    return match a_version_minor {
        VersionNum::Highest => {
            if a_version_major >= 1
                && a_version_major < RendererOpenGL::GL_MAX_VERSION_MINOR.len() as i32
            {
...
fn get_gles_version_minor(
    ...
    return match a_version_minor {
        VersionNum::Highest => {
            if a_version_major >= 1
                && a_version_major < RendererOpenGL::GLES_MAX_VERSION_MINOR.len() as i32
            {
