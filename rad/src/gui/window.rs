...
impl Window {
    pub fn new(
        ...
        let window = match a_renderer_type {
            ...
            renderer_types::RendererType::DirectX => Window::init_window(
                &video_subsystem,
                a_name,
                a_width as u32,
                a_height as u32,
            ...
            renderer_types::RendererType::Vulkan => Window::init_window_vulkan(
                &video_subsystem,
                a_name,
                a_width as u32,
                a_height as u32,
            ...
            renderer_types::RendererType::Metal => Window::init_window_metal(
                &video_subsystem,
                a_name,
                a_width as u32,
                a_height as u32,
    ...
    pub fn init_renderer(
        ...
        match a_renderer_type {
            renderer_types::RendererType::OpenGL | renderer_types::RendererType::OpenGLES => {
                Ok(Box::new(
                    match renderer_opengl::RendererOpenGL::new(
                        a_video_subsystem,
                        a_min_version,
                        a_max_version,
                        a_window,
            ...
            renderer_types::RendererType::DirectX => Ok(Box::new(
                match renderer_directx12::RendererDirectX12::new(
                    a_video_subsystem,
                    &a_window.window.lock().unwrap().inner,
                ) {
                    Ok(res) => res,
                    Err(_res) => return Err(WindowError::SdlRendererError),
                },
            )),
            renderer_types::RendererType::Vulkan => Ok(Box::new(
                match renderer_vulkan::RendererVulkan::new(a_window.window.clone(), true) {
                    Ok(res) => res,
                    Err(_res) => return Err(WindowError::SdlRendererError),
                },
            )),
            renderer_types::RendererType::Metal => Ok(Box::new(
                match renderer_metal::RendererMetal::new(a_video_subsystem, a_window) {
                    Ok(res) => res,
                    Err(_res) => return Err(WindowError::SdlRendererError),
                },
            )),
